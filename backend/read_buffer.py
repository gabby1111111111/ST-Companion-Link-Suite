...
class ReadBuffer:
    ...
    def __init__(self, ttl_seconds: int = DEFAULT_TTL_SECONDS):
        ...
        self._lock = threading.RLock()  # RLock 允许重入 (status -> get_keywords_summary)
        logger.info(f"📦 ReadBuffer 初始化 (TTL={ttl_seconds}s, Thread-Safe RLock)")
    ...
    def status(self) -> dict:
        ...
        with self._lock:
            ...
            return {
                ...
                # 递归调用 get_keywords_summary，因为使用了 RLock，所以是安全的
                "keywords_summary": self.get_keywords_summary(),
            }

    # ============================================================
    # 内部方法 (Caller must hold lock)
    # ============================================================
